...
public class StudentGradeController {
    ...
    public ResponseEntity<GradeListResponse> getSubjectGrades(
            @PathVariable Long classroomId,
            @PathVariable Long subjectId,
        ...
        return ResponseEntity.ok(
                studentGradeService.getSubjectGrades(
                        classroomId, subjectId, userDetails.getUserId(), academicYear, semester, examType));
    }

    // 담임 교사: 담당 학급 전 과목 성적 조회
    @GetMapping("/grades")
    public ResponseEntity<ClassroomGradeResponse> getClassroomGrades(
            @PathVariable Long classroomId,
            @RequestParam Integer academicYear,
            @RequestParam Integer semester,
            @RequestParam ExamType examType,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        return ResponseEntity.ok(
                studentGradeService.getClassroomGrades(
