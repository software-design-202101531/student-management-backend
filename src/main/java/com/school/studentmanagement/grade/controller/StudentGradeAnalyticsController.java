...
public class StudentGradeAnalyticsController {
    ...
    // 본인 성적 검색 — 과목(subjectId)·학기 범위 필터(모두 선택). 발행된 시험만.
    @GetMapping("/search")
    public ResponseEntity<ApiResponse<GradeSearchResponse>> searchMyGrades(
            @AuthenticationPrincipal CustomUserDetails userDetails,
        ...
        return ResponseEntity.ok(ApiResponse.ok(
                gradeSearchService.searchForStudent(userDetails.getUserId(),
