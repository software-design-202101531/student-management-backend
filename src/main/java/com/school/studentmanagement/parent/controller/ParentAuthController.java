...
public class ParentAuthController {
    ...
    @PostMapping("/verify")
    public ResponseEntity<ApiResponse<Long>> verifyParent(@Valid @RequestBody VerifyParentRequest request) {
        Long invitationId = parentAuthService.verifyParent(request);
        return ResponseEntity.ok(ApiResponse.ok(invitationId));
    }

    @PostMapping("/register")
    public ResponseEntity<ApiResponse<Void>> registerParent(@Valid @RequestBody ParentRegisterRequest request) {
